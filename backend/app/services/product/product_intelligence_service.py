...
class ProductIntelligenceService:
    @staticmethod
    def _extract_declaration_value(declarations: List[Dict[str, Any]], field_name: str) -> Optional[str]:
        for d in declarations:
            if d.get("field_name") == field_name:
                val = d.get("verified_value") or d.get("ai_value") or d.get("value")
                if val and str(val).strip():
                    return str(val).strip()
        return None

    @staticmethod
    def _extract_declaration_unit(declarations: List[Dict[str, Any]], field_name: str) -> Optional[str]:
        for d in declarations:
            if d.get("field_name") == field_name:
    ...
    async def identify_and_link_product(
        ...
        brand = self._extract_declaration_value(declarations, "brand_name") or self._extract_declaration_value(declarations, "brand")
        mfg_name = self._extract_declaration_value(declarations, "manufacturer_name")
        mfg_addr = self._extract_declaration_value(declarations, "manufacturer_address")
        ...
        barcode = self._extract_declaration_value(declarations, "barcode") or self._extract_declaration_value(declarations, "gtin")
        origin = self._extract_declaration_value(declarations, "country_of_origin") or "India"
        ...
        if qty and not unit:
            match = re.search(r"([0-9.]+)\s*([a-zA-Z]+)", qty)
            if match:
                qty = match.group(1)
                unit = match.group(2).upper()

        # If not enough identifiable data, cannot establish product identity
        if not commodity_name and not barcode:
            logger.info("Inspection %s lacks commodity name and barcode; product identity unestablished.", inspection_id)
            return None

        candidate_data = {
            "name": commodity_name or "Packaged Commodity",
            "brand": brand or "Packaged Commodity",
            "category": inspection.get("product_category") or "Packaged Commodity",
        ...
        if not matched_product and barcode:
            ...
            for p in all_prods:
                if p.get("barcode") and p.get("barcode") == barcode:
                    matched_product = p
                    break
        ...
        if not matched_product:
            ...
            for p in all_prods:
                ...
                if norm_name and norm_name == p_name and (not norm_brand or norm_brand in p_brand or p_brand in norm_brand):
                    matched_product = p
                    break
    ...
    async def sync_unlinked_inspections(self) -> int:
        """
        Finds all inspection cases in the repository where product_id is null,
        and automatically identifies and links them to real product records.
        """
        repo = get_repository()
        ...
        for ins in inspections:
            if not ins.get("product_id"):
                declarations = ins.get("declarations") or []
                images = ins.get("images") or []
                if declarations:
                    linked = await self.identify_and_link_product(ins, declarations, images)
                    if linked:
                        synced_count += 1

        if synced_count > 0:
            logger.info("Synchronized %d unlinked inspection cases to Product Intelligence.", synced_count)
        return synced_count
